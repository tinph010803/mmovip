...
<section class="col-lg-12 connectedSortable">
            <div class="card card-info">
                <div class="card-header">
                    <h3 class="card-title">GAME CHẴN LẺ 2</h3>
                </div>
                <div class="card-body">
                    <div class="card-body table-responsive p-0">
                        <table id="datatable1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                <?php 
  $MINHCHIEN = $MinhChien->query("SELECT * FROM `chanle` WHERE `id` >=3 ORDER BY id asc ");
  while ($row = mysqli_fetch_array($MINHCHIEN) ) 
  {?>
                                <tr class="text-center">
                                    <td><?=$row['id'];?></td>
                                    <td><?=$row['noidung'];?></td>
                                    <td><?=chanle($row['data']);?></td>
                                    <td><?=$row['min']; ?></td>
                                    <td><?=$row['max']; ?></td>
                                    <td><?=$row['tyle'];?></td>
            ...
            <div class="modal-body">
                <form role="form" action="" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" name="min" placeholder="20000">
                    </div>
                    <div class="form-group">
                        <label for="exampleInputEmail1">MAX</label>
                        <input type="number" class="form-control" name="max" placeholder="200000">
                    </div>
