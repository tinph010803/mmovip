...
<button class="codepro-custom-btn codepro-btn-6" target="blank" title="Nơi Nhập GiftCode VIP" onclick="window.open('/giftcode')"><span>Nhập GiftCode</span></button>
                        ...
                        <div class="panel panel-primary">
                                    <div class="table-responsive shadow">
                                        <table class="table table-striped table-bordered table-hover text-center">
                                          ...
                                          <?php
                $list_game = $MinhChien->get_list("SELECT * FROM `tong3so` WHERE `id` ORDER BY id asc ");
                foreach($list_game as $row) { ?>
                                            <tbody role="alert" aria-live="polite" aria-relevant="all" id="phone-tablechung" class="">
                                                <tr>
                                                    ...
                                                    <td id="677">
                                                        <b id="nearzekdev" style="position: relative;"><?=tong3so($row['data']);?></b>
                                                    </td>
                                                    <td id="677">
                        ...
                        <div class="panel panel-primary">
                                    <div class="table-responsive shadow">
                                        <table class="table table-striped table-bordered table-hover text-center">
                                            <thead>
                                                <tr role="row" class="bg-info">
                                                    <th class="text-center text-white bg-inf"><b>START GAME</b></th>
                                                </tr>
                                            </thead>
        ...
        document.write(a);
                                                            ...
                                                            <input class="form-control" type="text" id="noidung" placeholder="Tổng 3 số ( Nội Dung T1 or T2 or T3 )"><br>
                                                        </div>
                                                        <div class="form-group">
                                                            <label class="form-label" for="exampleInputEmail1">Số Tiền</label>
                                                            <input class="form-control" type="number" id="money" placeholder="5.000"><br>
                                                        </div>
                                                        <div>
                                                            <button type="submit" id="tong3so" class="btn btn-info">Xác Nhận</button>
                                                        </div>
                                                            </div>
                                                        </center>
                                                    </td>
...
$(document).ready(function(){
		$('#tong3so').click(function() {
		$('#tong3so').html('Đang Xử Lý...').prop('disabled', true);
		var formData = {
            'noidung'       : $("#noidung").val(),
            'money'         : $("#money").val()
	    };
		$.post("/api/tong3so", formData,
			function (data) {
			    if(data.status == 'error'){
				Swal.fire('Thông Báo', data.msg, data.status);
				$('#tong3so').html('Thanh Toán').prop('disabled', false);
			    } else {
