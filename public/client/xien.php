...
<button class="codepro-custom-btn codepro-btn-6" target="blank" title="Nơi Nhập GiftCode VIP" onclick="window.open('/giftcode')"><span>Nhập GiftCode</span></button>
                        ...
                        <div class="panel panel-primary">
                                    <div class="table-responsive shadow">
                                        <table class="table table-striped table-bordered table-hover text-center">
                                          ...
                                          <?php
                $list_game = $MinhChien->get_list("SELECT * FROM `xien` WHERE `id` ORDER BY id asc ");
                foreach($list_game as $row) { ?>
                                            <tbody role="alert" aria-live="polite" aria-relevant="all" id="phone-tablechung" class="">
                                                <tr>
                                                    ...
                                                    <td id="677">
                                                        <b id="nearzekdev" style="position: relative;"><?=xien($row['data']);?></b>
                                                    </td>
                                                    <td id="677">
        ...
        document.write(a);
                                                            ...
                                                            <input class="form-control" type="text" id="noidung" placeholder="Xiên ( Nội Dung CX or LT or CT or LX)"><br>
                                                        </div>
                                                        <div class="form-group">
                                                        ...
                                                        <div>
                                                            <button type="submit" id="xien" class="btn btn-info">Xác Nhận</button>
                                                        </div>
                                                            </div>
                                                        </center>
                                                    </td>
...
$(document).ready(function(){
		$('#xien').click(function() {
		$('#xien').html('Đang Xử Lý...').prop('disabled', true);
		var formData = {
            'noidung'       : $("#noidung").val(),
            'money'         : $("#money").val()
	    };
		$.post("/api/xien", formData,
			function (data) {
			    if(data.status == 'error'){
				Swal.fire('Thông Báo', data.msg, data.status);
				$('#xien').html('Thanh Toán').prop('disabled', false);
			    } else {
